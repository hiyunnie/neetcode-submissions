...
/**
# 我的想法
這題不能用XOR因為題目只說會出現不只一次，所以要用哈希表，先建一個哈希表，把全部數字放進去之後再看有沒有value !=1的。

# Intuition
一邊走訪陣列、一邊用 Set 紀錄看過的數字。若已在 Set 中即發現重複。時間複雜度O(n)

# Corner Cases
長度為 0 或 1、極端值、負數、資料結構的邊界問題。
...
# 我的想法
如果允許修改輸入，我會先排序，再比較相鄰元素。時間複雜度是 O(n log n)，額外空間依排序實作約 O(log n)，優點是 worst-case 較容易預估。如果不能修改輸入，我會使用預先配置的固定容量 hash table，透過 linear probing 處理 collision。平均時間是 O(n)，空間是 O(capacity)。若 table 滿了，API 應回傳明確的 capacity error，而不是誤判沒有 duplicate。
